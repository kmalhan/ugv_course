...
TEST(GpsSimTest, test) {
	...
	test_info("Received first speed command message, time starts now");

	ros::Time start_time = ros::Time::now();

	while (ros::ok() && !passed && !rollover) {

		ros::Duration total_time = ros::Time::now() - start_time;

	  try {
	    tf::StampedTransform transform;
	    listener_->lookupTransform("world", "base_footprint", ros::Time(0), transform);
